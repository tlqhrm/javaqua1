...
function frm_check(){
    ...
    if ($("input[name=address]").val()=="" || $("input[name=detailAddress]").val()=="" ){alert("주소를 입력해 주세요."); return false;}
    if($("input[name=chk1]").is(":checked") == false || $("input[name=chk2]").is(":checked") == false){alert("필수 이용약관동의에 체크해 주세요.");return false;}
    
    document.getElementById("totalAddress").value = $("input[name=address]").val() +" "+ $("input[name=detailAddress]").val();
    alert("회원가입이 완료되었습니다.");
